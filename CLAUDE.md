...
## 일괄 수정 검증 규칙 (부실 점검 방지)
- sed/grep 일괄 치환 후 **`grep -r '원본값'` 0건** 확인 필수 (주석, 문자열 포함)
- 대표 파일 1개가 아닌 **전 파일** 확인
- HTML + TSX + preview **모두** 확인 (구 방식 섹션별 TSX 포함)
- **"완료" 선언 전** 잔여 0건 + 적용 파일 수 일치 확인

...
<h2>What it does (in three lines)</h2>
  ...
  return (
    <>
      <style>{`
        ...
        .gaedl-hero__back:hover { color: var(--c-primary-dark, #3b2fbf); }
        ...
        .gaedl-body p:last-child { margin-bottom: 0; }
        ...
        .gaedl-body li strong {
          color: var(--c-ink, #0f1130);
          font-weight: 700;
        }

        .gaedl-body li:last-child { margin-bottom: 0; }

        .gaedl-body blockquote {
          margin: 28px 0;
          padding: 20px 24px;
        ...
        .gaedl-body pre::-webkit-scrollbar { display: none; }
        ...
        .gaedl-related__item:last-child {
          border-bottom: 1px solid var(--c-rule, #e5e7eb);
        }
        ...
        .gaedl-related__link:hover {
          color: var(--c-primary, #5b4fe9);
        }

        .gaedl-related__link-arrow {
          font-size: 16px;
          color: var(--c-primary, #5b4fe9);
          flex-shrink: 0;
        ...
        .gaedl-related__link:hover .gaedl-related__link-arrow {
          transform: translateX(3px);
        }

        /* ── Container queries ────────────────────────────── */
        @container (max-width: 767px) {
          .gaedl-hero { padding-top: 48px; }
          .gaedl-hero__title { font-size: 28px; }
          .gaedl-hero__lead { font-size: 16px; }
          .gaedl-def { padding: 24px 20px; border-radius: var(--r-md, 10px); }
          .gaedl-body p,
          .gaedl-body li { font-size: 16px; }
          .gaedl-body h2 { padding-top: 28px; }
        }
      `}</style>
      ...
      <div className="gaedl-root">

        {/* ── 1. Hero ── */}
        <section className="gaedl-hero">
          <div className="gaedl-container">
            <a href={backHref} className="gaedl-hero__back">{backLabel}</a>
            <h1 className="gaedl-hero__title">{term}</h1>
            <p className="gaedl-hero__lead">{lead}</p>
            <div className="gaedl-hero__meta">
              <span className="gaedl-meta__chip">{category}</span>
            </div>
          </div>
        </section>

        {/* ── 2. Definition block ── */}
        <div className="gaedl-def-wrap">
          <div className="gaedl-container">
            <div className="gaedl-def">
              <div className="gaedl-def__label">{definitionLabel}</div>
              <p className="gaedl-def__body">{definitionBody}</p>
            </div>
          </div>
        </div>

        {/* ── 3. Article Body ── */}
        <div className="gaedl-body-wrap">
          <div className="gaedl-container">
            <article
              className="gaedl-body"
        ...
        {relatedItems.length > 0 && (
          <div className="gaedl-related">
            <div className="gaedl-container">
              <div className="gaedl-related__label">Related terms</div>
              <ul className="gaedl-related__list">
                {relatedItems.map((item, i) => (
                  <li key={i} className="gaedl-related__item">
                    <a href={item.href} className="gaedl-related__link">
                      <span>{item.label}</span>
                      <span className="gaedl-related__link-arrow" aria-hidden="true">→</span>
                    </a>
                  </li>
                ))}
              </ul>

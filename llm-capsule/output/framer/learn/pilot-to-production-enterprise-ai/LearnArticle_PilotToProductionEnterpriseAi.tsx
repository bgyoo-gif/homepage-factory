...
// LearnArticle — Why Enterprise AI Pilots Stall — and How They Get to Production
// Article page: /learn/pilot-to-production-enterprise-ai
...
export default function LearnArticle_PilotToProductionEnterpriseAi({
  backLabel = "← Learn",
  backHref = "/learn",
    ...
    "mainEntity": [
      {
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Most enterprise AI pilots fail not because the AI is bad, but because the data layer between the AI and operational systems is missing. Pilots prove out on synthetic or anonymized data, then stall when the security, privacy, and compliance review opens. PII guardrails alone cannot protect structured operational data, and simple filtering cannot close residual compliance risk.",
        },
      },
      {
        "@type": "Question",
      ...
      {
        "@type": "Question",
        "name": "What is shadow AI and how does the data layer reduce it?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Shadow AI emerges when pilots stall — engineers paste anonymized snippets into consumer AI tools on personal devices to keep productivity gains they tasted in the pilot. When the official tooling provides the same AI capability through a secure data layer, shadow AI risk falls because the productivity becomes available through sanctioned channels.",
        },
      },
    ],
  })
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .ptpa-hero__back:hover { color: var(--c-primary-dark, #3b2fbf); }
        ...
        .ptpa-body p:last-child { margin-bottom: 0; }
        ...
        .ptpa-body li:last-child { margin-bottom: 0; }

        /* Blockquote */
        .ptpa-body blockquote {
          margin: 28px 0;
          padding: 20px 24px;
        ...
        .ptpa-body pre::-webkit-scrollbar { display: none; }
        ...
        .ptpa-body tr:last-child td { border-bottom: none; }
        ...
        .ptpa-body .takeaways ul {
          margin: 0;
          padding-left: 20px;
        }

        .ptpa-body .takeaways li {
          font-size: 15px;
        }
        ...
        .ptpa-body .ptpa-btn:hover { border-bottom: none; }

        .ptpa-body .ptpa-btn--primary {
          background-color: var(--c-ink, #0f1130);
          color: #ffffff;
        }
        .ptpa-body .ptpa-btn--primary:hover {
        ...
        @container (max-width: 767px) {
          .ptpa-hero { padding-top: 48px; }
          .ptpa-hero__title { font-size: 28px; }
          .ptpa-hero__lead { font-size: 16px; }
          .ptpa-tldr { padding: 24px 20px; border-radius: var(--r-md, 10px); }
          .ptpa-body p,
          .ptpa-body li { font-size: 16px; }
          .ptpa-body h2 { padding-top: 28px; }
          .ptpa-body table { font-size: 14px; display: block; overflow-x: auto; scrollbar-width: none; }
          .ptpa-body table::-webkit-scrollbar { display: none; }
          .ptpa-cta__inner { gap: 10px; }
          .ptpa-cta__title { font-size: 24px; }
          .ptpa-cta__desc { font-size: 15px; }
        }
      `}</style>

      <div className="ptpa-root">

        {/* ── 1. Article Hero ── */}
        <section className="ptpa-hero">
          <div className="ptpa-container">
            <a href={backHref} className="ptpa-hero__back">{backLabel}</a>
            <h1 className="ptpa-hero__title">{title}</h1>
            <p className="ptpa-hero__lead">{lead}</p>
            <div className="ptpa-hero__meta">
              <span className="ptpa-meta__chip">{category}</span>
              <span className="ptpa-meta__sep" aria-hidden="true" />
              <span className="ptpa-meta__time">{readTime}</span>
              <span className="ptpa-meta__sep" aria-hidden="true" />
              <span className="ptpa-meta__date">{dateUpdated}</span>
            </div>
          </div>
        </section>

        {/* ── 2. TL;DR ── */}
        <div className="ptpa-tldr-wrap">
          <div className="ptpa-container">
            <div className="ptpa-tldr">
              <div className="ptpa-tldr__label">{tldrLabel}</div>
              <p className="ptpa-tldr__body">{tldrBody}</p>
            </div>
          </div>
        </div>

        {/* ── 3. Article Body ── */}
        <div className="ptpa-body-wrap">
          <div className="ptpa-container">
            <article
              className="ptpa-body"
        ...
        {relatedItems.length > 0 && (
          <div className="ptpa-related">
            <div className="ptpa-container">
              <div className="ptpa-related__label">{relatedSectionLabel}</div>
              <div className="ptpa-related__grid">
                {relatedItems.map((item, i) => (
                  <a key={i} href={item.href} className="ptpa-related__card">
                    <span className="ptpa-related__card-title">{item.title}</span>
                    <span className="ptpa-related__card-arrow" aria-hidden="true">→</span>
                  </a>
                ))}
              </div>
            </div>
        ...
        {/* ── 5. CTA Strip ── */}
        <div className="ptpa-cta">
          <div className="ptpa-container">
            <div className="ptpa-cta__inner">
              <h2 className="ptpa-cta__title">{ctaTitle}</h2>
              {ctaDescription && (
                <p className="ptpa-cta__desc">{ctaDescription}</p>
              )}
              <a href={ctaHref} className="ptpa-cta__btn">{ctaLabel}</a>
            </div>
          </div>
        </div>
...
addPropertyControls(LearnArticle_PilotToProductionEnterpriseAi, {
  // Hero
  ...
  // SEO
  canonicalUrl:  { type: ControlType.String, title: "Canonical URL",   defaultValue: "https://llmcapsule.ai/learn/pilot-to-production-enterprise-ai" },
  datePublished: { type: ControlType.String, title: "Date Published",  defaultValue: "2025-04-15" },

  // Related links
  relatedSectionLabel: { type: ControlType.String, title: "Related Section Label", defaultValue: "Related" },

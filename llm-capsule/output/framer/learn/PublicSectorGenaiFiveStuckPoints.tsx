...
<figure style="margin: 32px 0;">
  <svg width="100%" viewBox="0 0 720 580" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="100" y="80" font-family="Inter, sans-serif" font-size="10" fill="#5b4fe9" text-anchor="middle">N2SF 1~2단계</text>
    ...
    <text x="240" y="80" font-family="Inter, sans-serif" font-size="10" fill="#5b4fe9" text-anchor="middle">N2SF 2단계</text>
    ...
    <text x="380" y="80" font-family="Inter, sans-serif" font-size="10" fill="#5b4fe9" text-anchor="middle">N2SF 3단계</text>
    ...
    <text x="640" y="80" font-family="Inter, sans-serif" font-size="10" fill="#5b4fe9" text-anchor="middle">N2SF 5단계</text>
...
<h3>실제 어떻게 막히는가</h3>
<p>N2SF 절차의 두 번째 단계가 정보 등급 분류(C/S/O)입니다. 정책 문서에는 한 줄로 적혀 있습니다. <strong>"기관이 보유한 정보자산을 등급별로 분류한다."</strong> 그런데 막상 시작하면 첫 번째 질문이 떠오릅니다.</p>
...
<div class="callout">
  ...
  <div class="callout__body">N2SF는 C/S/O 세 등급을 정의하지만, 실무에서 <strong>C 등급(기밀)은 일반적으로 시스템에 올라가 있지 않고 별도 USB·격리된 보관소 등에 보관</strong>됩니다. 즉 전자결재·문서관리 시스템상에서 분류 대상이 되는 정보는 C 등급을 제외한 S/O 이분입니다. 이 점을 인식하면 분류 작업의 실제 부담이 시각적으로 줄어듭니다.</div>
</div>

<h3>왜 그렇게 막히는가</h3>
<p>이 막힘의 본질은 정책 절차가 아니라 <strong>물리적 규모</strong>입니다. N2SF 가이드라인은 등급 분류의 원칙은 명확히 제시하지만, "수백만 건을 어떻게 효율적으로 분류할 것인가"의 실행 방법은 다루지 않습니다.</p>
...
<h3>풀어내는 방향</h3>
<p>후보가 적다고 마스킹 솔루션으로 타협하는 것은 위험합니다. 단순 마스킹 기반 솔루션은 운영 단계에서 보안 우려가 재발생하거나, 감사 단계에서 N2SF 정합성 논증이 도전받을 수 있습니다.</p>
<p>대안은 다음과 같습니다.</p>
<ul>
...
<p>이게 의미하는 바는 분명합니다. N2SF와 모델 2가 제시한 정책 절차는 안전한 도입의 <strong>최소 기준선</strong>이지, 도입 성공을 보장하는 종합 가이드가 아닙니다. 정책 절차를 따르되, 그 절차가 우리 기관의 물리적 규모·활용 현실·시장 상황과 만났을 때 어떻게 변형되어야 하는지를 각 기관이 스스로 설계해야 합니다.</p>
...
<p>모두 동시에 풀려고 하면 사업이 한없이 길어집니다. 도입 단계별로 우선 풀어야 할 지점이 다르므로, 단계별로 가장 큰 막힘을 식별해서 그 지점을 우선 풀어내는 접근이 효율적입니다. 예를 들어 N2SF 1~2단계에서는 지점 1·2가, 솔루션 평가 단계에서는 지점 3이 우선입니다.</p>

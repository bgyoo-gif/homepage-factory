...
<h2>1. 왜 지금 N2SF인가 — 망분리의 한계</h2>
<p>N2SF를 이해하려면 먼저 기존 체계가 왜 한계에 부딪혔는지부터 살펴봐야 합니다.</p>
...
<p>국가정보원과 국가보안기술연구소(NSR)는 이런 시대적 변화를 인식하고, 망분리를 대체할 새로운 보안 체계를 설계했습니다. 그 결과물이 N2SF입니다.</p>

<h2>2. N2SF의 정의와 핵심 개념</h2>
<p>N2SF는 <strong>National Network Security Framework</strong>의 약자로, 한국어로는 <strong>국가 망 보안체계</strong>로 번역됩니다. 한 줄로 정의하면 다음과 같습니다.</p>

<blockquote>정보의 중요도와 시스템의 역할에 따라 보안 통제를 차등 적용하여, 신기술 활용과 보안을 동시에 달성하는 공공부문 보안 프레임워크.</blockquote>

<p>이 정의에서 핵심은 <strong>"차등 적용"</strong>입니다. 모든 정보와 시스템에 동일한 통제를 적용하던 망분리 시대와 달리, N2SF는 정보의 성격에 따라 통제의 강도를 다르게 가져갑니다. 가장 민감한 정보에는 가장 강한 통제를, 덜 민감한 정보에는 그에 맞는 적정 수준의 통제를 적용합니다.</p>
<p>이런 사고 방식을 보안 업계에서는 <strong>다중계층보안(MLS, Multi-Layered Security)</strong>이라고 부릅니다. 단일한 경계로 모든 것을 차단하는 대신, 정보 자산의 가치와 위협 모델에 따라 여러 층의 보호를 차등적으로 적용하는 접근입니다. 미국 국방부의 정보 분류 체계, NATO의 보안 등급 체계, EU의 EU Restricted/Confidential/Secret 분류 등이 모두 비슷한 사고에 기반하고 있습니다. N2SF는 이런 국제적 흐름을 한국 공공부문 환경에 맞게 구체화한 결과물입니다.</p>

<h3>N2SF의 3대 구성 요소</h3>
<p>N2SF를 실무에 적용할 때는 다음 세 가지를 차례로 다루게 됩니다.</p>
<ol>
...
<h2>3. C/S/O 등급 — N2SF의 기본 분류 체계</h2>
<p>N2SF의 출발점은 정보 등급 분류입니다. 모든 업무 정보는 다음 세 등급 중 하나에 해당합니다.</p>

<figure style="margin: 32px 0;">
  <svg width="100%" viewBox="0 0 680 460" xmlns="http://www.w3.org/2000/svg">
    <title>C/S/O 정보 등급 분류 체계</title>
    <desc>N2SF의 C, S, O 세 등급의 정의, 정보 예시, 외부 연계 허용 수준 비교</desc>
...
<h2>4. 「위치-주체-객체」 모델링 — N2SF의 사고 방식</h2>
<p>정보를 등급으로 분류했다면, 다음 단계는 <strong>"이 정보를 어떻게 사용할 것인가"</strong>를 분석하는 것입니다. N2SF는 이를 위해 「위치-주체-객체」라는 분석 모델을 제시합니다.</p>
...
<figure style="margin: 32px 0;">
  <svg width="100%" viewBox="0 0 680 440" xmlns="http://www.w3.org/2000/svg">
    <title>「위치-주체-객체」 모델링 — N2SF의 보안 분석 사고 방식</title>
    <desc>N2SF가 정보서비스를 분석할 때 사용하는 위치, 주체, 객체 세 축의 의미와 평가 예시</desc>
    <defs>
...
<p>바로 이 분석 프레임워크가 N2SF의 가장 중요한 사고 도구입니다. 새로운 시스템이나 서비스를 도입할 때, 「위치-주체-객체」를 그려보고 등급을 부여하면, 어떤 보안 통제가 필요한지 체계적으로 도출할 수 있습니다.</p>

<h2>5. 보안원칙 — 정보 생산·저장과 정보 이동</h2>
<p>「위치-주체-객체」 모델링이 분석 도구라면, 보안원칙은 그 분석 결과에 적용되는 판단 기준입니다. N2SF는 두 가지 핵심 보안원칙을 제시합니다.</p>
...
<div class="callout">
  <div class="callout__icon">💡</div>
  <div class="callout__body"><strong>두 원칙이 함께 작동하는 방식</strong> 「위치-주체-객체」로 시나리오를 모델링한 뒤, 두 보안원칙에 비추어 어떤 부분이 원칙에 부합하고 어떤 부분에 추가 통제가 필요한지 판단합니다. 이것이 N2SF 보안 설계의 기본 흐름입니다.</div>
</div>

<h2>6. 적용 단계 — 우리 기관은 무엇부터 해야 하나</h2>
<p>N2SF는 추상적 정책 프레임워크지만, 실무 적용 시에는 구체적인 단계가 있습니다. 「N2SF 보안 가이드라인」은 정보서비스 도입·운영 과정을 다음 5단계로 정리합니다.</p>
<ol>
...
<h2>7. N2SF가 열어준 가능성과 새로운 질문들</h2>
<p>N2SF의 도입은 공공기관에게 <strong>새로운 가능성</strong>을 열어주었습니다. 그동안 망분리 원칙 때문에 불가능했던 시도들이 이제 가능해졌습니다.</p>
<ul>
...
<p>이런 질문들은 N2SF가 완성된 답안이 아니라 <strong>새로운 사고 프레임워크</strong>이기 때문에 발생합니다. 프레임워크는 길을 보여주지만, 그 길을 어떻게 걸을지는 각 기관이 결정해야 합니다. 그리고 이 결정은 단순히 보안 부서만의 일이 아니라, 정보화 정책, 업무 프로세스, 외부 솔루션 선정까지 연결되는 통합적 의사결정입니다.</p>
<p>특히 생성형 AI 도입은 N2SF 적용에서 가장 활발히 논의되는 영역입니다. 국가정보원과 NSR은 이 영역을 위해 별도의 「업무환경에서 생성형 AI 활용 모델 해설서(모델 2)」를 발간했으며, 21개의 보안위협과 50여 개의 보안통제 항목을 구체적으로 제시하고 있습니다. 다음 글에서는 이 모델 2를 심층 해설하겠습니다.</p>

<div class="takeaways">
  <div class="takeaways__label">이 글의 핵심 요약</div>
  <ul>
    <li>N2SF는 망분리에서 다중계층보안(MLS)으로의 패러다임 전환입니다</li>
...
<p>아닙니다. C 등급 정보를 다루는 시스템에는 망분리 수준의 강한 통제가 여전히 적용됩니다. N2SF가 폐지한 것은 "모든 정보에 일률적으로 망분리를 적용"하는 방식이며, 정보 등급에 따라 적절한 수준의 통제를 차등 적용하는 방식으로 진화한 것입니다. 즉, 강한 보안이 필요한 곳에는 여전히 강한 보안이, 그렇지 않은 곳에는 적정 수준의 보안이 적용됩니다.</p>
...
<h3>모델 해설서는 무엇인가요?</h3>
<p>국가정보원과 국가보안기술연구소(NSR)가 발간하는 「정보서비스 모델 해설서」 시리즈는 대표적인 정보서비스 시나리오에 N2SF를 적용한 분석 예시입니다. 모델 1은 일반 정보서비스, 모델 2는 업무환경에서 생성형 AI 활용을 다루고 있으며, 향후 더 많은 모델이 추가될 예정입니다. 각 기관은 자신의 시나리오와 유사한 모델을 참고하여 적용 절차를 설계할 수 있습니다.</p>

<h3>N2SF 적용에 비용이 많이 드나요?</h3>
<p>적용 자체에는 직접 비용이 들지 않지만, 정보자산 분류, 시스템 모델링, 보안 통제 도입 등의 작업에는 시간과 자원이 필요합니다. 기존 망분리 시스템을 유지하는 것보다 N2SF 기반으로 재설계하는 것이 단기적으로는 더 큰 작업이 될 수 있습니다. 다만 장기적으로는 클라우드·AI 활용을 통한 효율성 향상이 가능해지므로, 적용 비용을 상쇄하고 남는 가치가 있습니다.</p>
...
const FAQ_JSON_LD = `{ "@context": "https://schema.org", "@type": "FAQPage", "mainEntity": [ {"@type":"Question","name":"N2SF는 의무 적용인가요?","acceptedAnswer":{"@type":"Answer","text":"국가정보원이 발간한 「국가 망 보안체계 보안 가이드라인」은 공공기관의 정보서비스 구축·운영 시 적용 기준을 제시합니다. 모든 기관이 즉시 전면 적용해야 하는 강행 규정의 형식은 아니지만, 신규 정보화 사업의 보안성 검토에서 사실상의 표준으로 작동하고 있습니다."}}, {"@type":"Question","name":"망분리는 완전히 폐지되나요?","acceptedAnswer":{"@type":"Answer","text":"아닙니다. C 등급 정보를 다루는 시스템에는 망분리 수준의 강한 통제가 여전히 적용됩니다. N2SF가 폐지한 것은 모든 정보에 일률적으로 망분리를 적용하는 방식입니다."}}, {"@type":"Question","name":"C/S/O 분류는 누가 정하나요?","acceptedAnswer":{"@type":"Answer","text":"각 기관이 자체적으로 정보자산을 식별하고 등급을 분류합니다. 다만 일부 영역은 외부 기준이 적용됩니다."}}, {"@type":"Question","name":"모델 해설서는 무엇인가요?","acceptedAnswer":{"@type":"Answer","text":"국가정보원과 국가보안기술연구소(NSR)가 발간하는 「정보서비스 모델 해설서」 시리즈는 대표적인 정보서비스 시나리오에 N2SF를 적용한 분석 예시입니다."}}, {"@type":"Question","name":"N2SF 적용에 비용이 많이 드나요?","acceptedAnswer":{"@type":"Answer","text":"적용 자체에는 직접 비용이 들지 않지만, 정보자산 분류, 시스템 모델링, 보안 통제 도입 등의 작업에는 시간과 자원이 필요합니다."}} ] }`
...
addPropertyControls(WhatIsN2sf, {
  ...
  title: { type: ControlType.String, title: "Title", defaultValue: "N2SF란 무엇인가 — 공공기관 보안의 새 패러다임 완벽 정리" },
  lead: { type: ControlType.String, title: "Lead", defaultValue: "국가 망 보안체계(N2SF)는 망분리에서 다중계층보안(MLS)으로의 전환입니다. C/S/O 등급 체계, 「위치-주체-객체」 모델링, 보안원칙을 처음부터 끝까지 정리합니다.", displayTextArea: true },
  ...
  tldrBody: { type: ControlType.String, title: "TL;DR Body", defaultValue: "N2SF는 공공부문 보안의 패러다임 전환입니다. 모든 외부 자원을 일률 차단하던 망분리에서, 정보를 C(기밀)·S(민감)·O(공개) 세 등급으로 분류하고 등급별로 다른 통제를 적용하는 다중계층보안으로 전환했습니다. 이 전환은 외부 생성형 AI 활용의 길을 열되, 「위치-주체-객체」 모델링·정보 이동 보안원칙·21개 보안위협 식별·50여 개 보안통제 항목이라는 구체적인 프레임워크 안에서만 가능합니다.", displayTextArea: true },
  ...
  breadcrumbLabel: { type: ControlType.String, title: "Breadcrumb Label", defaultValue: "N2SF란 무엇인가" },

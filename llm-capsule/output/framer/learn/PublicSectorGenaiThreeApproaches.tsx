...
<p>2024~2025년 발표된 N2SF(National Network Security Framework, 국가 망 보안체계)는 이 패러다임을 <strong>다중계층보안(MLS, Multi-Layered Security)</strong>으로 전환했습니다. 핵심은 정보를 C(Classified, 기밀)·S(Sensitive, 민감)·O(Open, 공개)의 세 등급으로 분류하고, 등급별로 다른 보안 통제를 적용하는 방식입니다. 모든 정보를 무차별 차단하는 것이 아니라, 정보의 등급에 맞는 보호를 적용하는 것입니다.</p>

<figure style="margin: 32px 0;">
  <svg width="100%" viewBox="0 0 680 380" role="img" xmlns="http://www.w3.org/2000/svg">
    <title>N2SF 패러다임 전환 — 물리적 망분리에서 다중계층보안으로</title>
    <desc>기존 망분리 방식과 N2SF 다중계층보안 방식의 차이를 비교한 도식</desc>
    <defs>
    ...
    <text x="380" y="32" font-family="Inter, sans-serif" font-size="14" font-weight="500" fill="#0f1130">N2SF — 다중계층보안 (MLS)</text>
    ...
    <text x="40" y="354" font-family="Inter, sans-serif" font-size="13" fill="#3a3d5e">N2SF: 정보 등급과 보안 통제에 따라 외부 AI 활용 가능</text>
  </svg>
  <figcaption style="text-align: center; font-size: 13px; color: #6b7280; margin-top: 8px;">도식 1. N2SF 패러다임 전환</figcaption>
</figure>
...
<h3>2.2 장점</h3>
<p>이 방식의 가장 큰 장점은 <strong>N2SF 보안원칙 위배 가능성의 원천 차단</strong>입니다. 외부로 나가는 정보가 없으니 정보 이동 원칙을 어길 일도 없고, 등급 평가의 복잡성도 발생하지 않습니다. 감사 대응 논리도 단순합니다. "우리 기관은 외부 AI 사용을 차단했다"는 한 줄로 끝납니다.</p>
...
<h3>3.2 장점</h3>
<p>가장 큰 장점은 <strong>완전한 데이터 통제</strong>입니다. 입력 프롬프트와 출력 응답 모두 기관 내부에서만 처리되므로, 외부 유출 가능성이 구조적으로 0입니다. N2SF 정합성 측면에서도 「위치-주체-객체」가 모두 S등급 내부망에 위치하므로 정보 이동 원칙 위배가 발생하지 않습니다.</p>
...
<p>이 방식의 정보 흐름은 「위치(기관 전산망 S) - 주체(이용자 단말 S) - 객체(외부 생성형 AI O)」 모델에 해당하며, 정확히 N2SF 모델 2가 다루는 시나리오입니다.</p>
...
<ul>
  ...
  <li><strong>N2SF 정합성</strong> — 게이트웨이가 제공하는 보안 통제는 모델 2가 요구하는 AI 연계체계의 핵심 기능들과 직접 대응됩니다. 이용자·단말 인증, 콘텐츠 필터링, 보안등급 식별, Proxy 강제화, 일방향 정보흐름 통제, 감사 로그 등이 게이트웨이의 기본 기능입니다.</li>
</ul>
...
<p>이런 한계 때문에, 마스킹 기반 게이트웨이는 <strong>N2SF 정합성 논증에 약점</strong>을 가집니다. "민감 정보를 마스킹했으니 안전하다"는 주장이 감사 단계에서 도전받을 수 있습니다.</p>
...
<p>이 기법이 적용된 게이트웨이는 단순 마스킹 게이트웨이와 <strong>본질적으로 다른 보안 수준</strong>을 제공합니다. N2SF 정합성 논증에서도 학술적 권위에 근거한 정당화가 가능해집니다.</p>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>N2SF 정합성</td><td>위배 없음</td><td>위배 없음</td><td>모델 2 준수 (보호 방식 검증 필요)</td></tr>
...
<div class="callout">
  ...
  <div class="callout__body"><strong>게이트웨이 솔루션 평가 시 반드시 던져야 할 질문 5가지</strong>
    <ol style="margin-top: 12px; margin-bottom: 0;">
      ...
      <li>N2SF 모델 2의 보안통제 항목 중 어느 항목들을 어떻게 충족하는가?</li>
      <li>보호 처리 후 데이터의 AI 활용 품질(utility)은 어떻게 검증되었는가?</li>
    </ol>
  </div>
</div>

<p>이 질문에 명확히 답할 수 있는 솔루션만이 기관 도입 검토 대상이 됩니다. 위 질문들의 답이 모호하다면, 그 솔루션은 N2SF 환경에서 지속 가능한 선택이 되기 어렵습니다.</p>
<p>공공기관의 생성형 AI 도입은 이제 "할 것인가"의 문제가 아니라, "어떤 길로 갈 것인가"의 문제입니다. 그리고 그 길의 끝에 무엇이 있는지는, 길을 떠나기 전 충분한 분석에서 시작됩니다.</p>

<h2>자주 묻는 질문</h2>

<h3>AI DLP만으로도 N2SF 충족이 가능한가요?</h3>
<p>가능합니다. 외부 AI 사용을 차단하면 정보 이동 자체가 발생하지 않으므로 N2SF 보안원칙 위배가 없습니다. 다만 이는 AI 활용 가치를 포기하는 대가로 얻는 안전성이며, 섀도우 AI 문제로 인해 실효적 통제가 약화될 수 있다는 점은 별도로 고려해야 합니다.</p>
...
<p>가장 큰 약점은 보호 방식의 수준이 게이트웨이마다 다르다는 점입니다. 단순 마스킹 기반 게이트웨이는 재식별 공격에 취약하고 N2SF 정합성 논증이 약합니다. 차등정보보호 같은 수학적으로 검증된 보호 기법을 사용하는 게이트웨이만이 본 방식의 가치를 온전히 실현할 수 있습니다.</p>
...
export default function PublicSectorGenaiThreeApproaches({
  ...
  related2Title = "N2SF란 무엇인가 — 공공기관 보안의 새 패러다임 완벽 정리",
...
addPropertyControls(PublicSectorGenaiThreeApproaches, {
  ...
  title: { type: ControlType.String, title: "Title", defaultValue: "공공기관 생성형 AI 도입의 세 가지 길" },
  lead: { type: ControlType.String, title: "Lead", defaultValue: "AI DLP·차단, sLLM 자체구축, 게이트웨이 방식 — N2SF 시대 공공기관의 선택지를 비용·성능·보안·정합성 관점에서 객관적으로 비교합니다.", displayTextArea: true },
  ...
  tldrBody: { type: ControlType.String, title: "TL;DR Body", defaultValue: "공공기관의 생성형 AI 도입은 AI DLP·차단, sLLM 자체구축, 게이트웨이 방식의 세 갈래로 나뉩니다. 차단 방식은 안전하지만 활용을 포기해야 하고, sLLM 자체구축은 7~13억 원의 초기 비용에 더해 최신 상용 모델과의 성능 격차가 누적됩니다. 게이트웨이 방식은 비용과 성능의 균형을 잡지만, 내부 보호 기법의 수준이 N2SF 정합성을 결정합니다. 단순 마스킹은 재식별 공격에 취약하며, 차등정보보호(Differential Privacy) 기반의 게이트웨이가 수학적으로 검증된 유일한 답에 가깝습니다.", displayTextArea: true },
  bodyHtml: { type: ControlType.String, title: "Body HTML", defaultValue: BODY_HTML, displayTextArea: true },
  canonicalUrl: { type: ControlType.String, title: "Canonical URL", defaultValue: "https://llmcapsule.ai/resources/learn/public-sector-genai-three-approaches-in-korea" },
  ...
  breadcrumbLabel: { type: ControlType.String, title: "Breadcrumb Label", defaultValue: "공공기관 생성형 AI 도입의 세 가지 길" },
  ...
  related2Title: { type: ControlType.String, title: "Related 2 Title", defaultValue: "N2SF란 무엇인가 — 공공기관 보안의 새 패러다임 완벽 정리" },

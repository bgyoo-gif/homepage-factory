...
| "구체적이고 분리된 동의" | 1건 |
| "C (Classified)" + "S (Sensitive)" + "O (Open)" — N²SF 등급 체계 갱신 (TS→O) | 각 2건 |
...
1. **N²SF 등급 체계**: C/S/TS → **C/S/O** (Open 등급 추가, TS 삭제)
2. **N²SF 모델 도입**: 모델 1(내부망 폐쇄)/모델 2(외부 상용)/모델 3(하이브리드) 개념 추가
3. **국외이전 표 축소**: 5행 → **3행** (적용 범위 / 데이터 위치 / 법적 함의)
4. **N²SF 표 갱신**: 등급 × 모델 2 활용 가능성 × ChatGPT Teams 활용 시 검토 사항 (3행)
...
| 3. N²SF 등급 체계와의 정합성 | section-3 | + ds-bullet--dot + ds-table (3행) |
...
**PASS** — Critical/High 결함 0건. revised 본문 톤다운·구조 변경(N²SF 등급 C/S/O, 모델 1/2/3, 옵션 A/B/C) 모두 반영. 다음 단계: bodyhtml 재추출 + build-learn-tsx.py 엔트리 갱신 + TSX 재빌드.

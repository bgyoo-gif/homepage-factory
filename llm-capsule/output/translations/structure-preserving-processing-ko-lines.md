# Structure-Preserving Processing — 한국어 번역
...
민감 요소를 대체하면서 정확한 AI 처리를 위해 문서 레이아웃, 엔티티 관계, 의미적 맥락을 유지합니다 — 다음의 핵심 기능입니다 —
...
문서 구조 보존 처리는 문서 레이아웃, 엔티티 관계, 테이블 구조, 상호 참조, 의미적 맥락을 유지하면서 기업 문서의 민감 요소를 대체하는 데이터 보호 방식입니다. AI 모델이 정확한 처리를 위해 구조적으로 온전한 문서를 수신하도록 보장합니다.
...
평면 마스킹은 모든 민감 값을 독립적인 토큰으로 취급하여 각각을 일반 자리 표시자로 대체합니다. 이는 엔티티 관계를 붕괴시키고, 테이블 스키마를 파괴하며, AI 모델이 이해에 사용하는 구조적 정보를 제거합니다.
...
문서 구조 보존 처리는 이러한 관계를 유지합니다. 같은 엔티티가 여러 위치에 나타나면 모든 항목이 같은 대체 값에 매핑됩니다. 테이블 열 헤더와 행 식별자는 의미적으로 일관성을 유지합니다. 문서 섹션 간 상호 참조는 논리적 연결을 유지합니다.
...
A contract references multiple parties across clauses. Structure-preserving processing assigns ENTITY_A to the acquirer throughout the entire document — maintaining consistent entity tracking for AI analysis.
계약서가 조항 전반에 걸쳐 여러 당사자를 참조합니다. 문서 구조 보존 처리는 전체 문서에서 인수자에게 ENTITY_A를 할당하여 — AI 분석을 위한 일관된 엔티티 추적을 유지합니다.
...
캡슐화 및 AI 처리 파이프라인 전체에서 기업 문서가 완전한 구조적 무결성을 유지하는 방법을 직접 확인하세요.

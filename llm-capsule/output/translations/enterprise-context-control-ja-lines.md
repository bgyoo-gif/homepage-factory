# Enterprise Context Control — 日本語翻訳
...
AI処理中に文書タイプ、部門、ワークフローのコンテキストに基づいてどのデータを保護するかを決定する、ポリシーベースの機密性分類です。
...
エンタープライズコンテキスト制御は、AIワークフローにおける機密性分類のポリシーベースのアプローチです。すべてのデータに均一なルールを適用するのではなく、文書タイプ、部門の出所、ワークフローステージ、企業固有の機密性ポリシーに基づいて保護対象を決定します。コンテキスト認識型データ制御のコアコンポーネントです。
...
すべてのデータがあらゆるコンテキストで同じ機密性を持つわけではありません。プレスリリースにおける企業名は公開情報です。M&A対象リストにおける同じ企業名は非常に機密性が高くなります。エンタープライズコンテキスト制御により、組織はこのようなコンテキストの違いを反映した保護ポリシーを定義できます。
...
これはPII検出を超えるものです。エンタープライズコンテキスト制御は、営業秘密、金融条件、戦略データ、内部指標、および組織固有のあらゆる機密性カテゴリを対象とし、真のエンタープライズ機密性制御を提供します。
...
銀行が3種類の文書に対してそれぞれ異なるカプセル化ポリシーを定義します。顧客書簡（PIIのみ保護）、内部リスク評価（PIIに加えリスクスコアと限度額の推奨を保護）、取締役会プレゼンテーション（すべての財務予測と戦略的コメントを保護）。エンタープライズコンテキスト制御が各文書に適切なポリシーを自動的に適用します。
...
ポリシーベースの機密性分類が、あらゆる文書タイプとワークフローにわたって企業データをどのように保護するかをご体験ください。

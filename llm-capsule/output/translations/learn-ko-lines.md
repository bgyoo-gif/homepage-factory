# Learn Page — 한국어 번역

## Section 01: Hero

Context-Preserving Data Layer for AI — Learn
기업 AI 도입 — Learn

In-depth technical articles on enterprise context-preserving data layer for AI, secure AI workflows, structure-preserving processing, and restorable workflows.
기업 AI 도입, 보안 AI 워크플로우, 구조 보존 처리, 복원 가능 워크플로우에 대한 심층 기술 아티클을 제공합니다.

## Section 02: Article Cards

Secure Enterprise AI Data Workflows
기업 AI 데이터 워크플로우 보안

Pillar
핵심 가이드

Context-Preserving Data Layer for AI
기업 AI 도입

Enterprise AI Document Processing
기업 AI 문서 처리

What Is an AI Data Capsule
AI 데이터 캡슐이란

Article
아티클

How to Use AI on Sensitive Enterprise Data
민감한 기업 데이터에서 AI를 사용하는 방법

Why Redaction Breaks Enterprise AI Workflows
삭제(Redaction)가 기업 AI 워크플로우를 망치는 이유

Featured
주요 아티클

Structure-Preserving Document Processing
구조 보존 문서 처리

PII Protection vs Enterprise Confidentiality Control
PII 보호와 기업 기밀 통제의 차이

On-Premise vs Cloud AI Data Protection
온프레미스와 클라우드 AI 데이터 보호 비교

Local Restoration vs Anonymization
로컬 복원과 익명화 비교

AI Data Pipeline Protection
AI 데이터 파이프라인 보호

Restorable Workflow
복원 가능 워크플로우

Glossary
용어 사전

Enterprise Context Control
조직 정의 민감정보 통제

LLM Capsule vs Masking Tools
LLM Capsule vs 마스킹 툴

Comparison
비교

LLM Capsule vs Prompt Security Gateways
LLM Capsule vs 프롬프트 보안 게이트웨이

LLM Capsule vs Synthetic Data Platforms
LLM Capsule vs 합성 데이터 플랫폼

Structure-Preserving Processing vs Flat Masking
구조 보존 처리 vs 단순 마스킹

Read article
아티클 읽기

## Section 03: CTA Band

See how LLM Capsule works with your data
LLM Capsule이 실제 데이터에서 어떻게 작동하는지 직접 확인하세요

Bring your documents, deployment constraints, and evaluation criteria. We demonstrate on your actual workflows.
실제 문서, 배포 조건, 평가 기준을 가져오세요. 실제 워크플로우에서 직접 시연합니다.

Request a Demo
데모 요청

Available on AWS Marketplace
AWS 마켓플레이스에서 보기

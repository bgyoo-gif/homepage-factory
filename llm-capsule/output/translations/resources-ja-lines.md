# Resources Page — 日本語翻訳

## Section 01: Hero

Context-Preserving Data Layer for AI
エンタープライズAI導入

Resources
リソース

Technical documentation, learning resources, glossary, and trust information for enterprise context-preserving data layer for AI with LLM Capsule.
LLM Capsuleを活用したエンタープライズAI導入のための技術文書、学習リソース、用語集、信頼性情報を提供いたします。

## Section 02: Resource Hub

Explore
探索

Resources
リソース

Browse documentation, learning materials, key terms, and trust information.
技術文書、学習資料、主要用語、信頼性情報をご覧ください。

Learn
Learn

In-depth articles on enterprise context-preserving data layer for AI, secure LLM usage, structure-preserving processing, and restorable workflows.
エンタープライズAI導入、セキュアLLM活用、構造保存処理、復元可能ワークフローに関する詳細な記事を提供いたします。

Browse Learn
Learnを探索

Glossary
用語集

Definitions of key concepts — restoration, zero exposure, enterprise context control, and structure-preserving processing.
主要概念の定義 — 復元、Zero Exposure、エンタープライズコンテキスト制御、構造保存処理について説明いたします。

Browse Glossary
用語集を探索

Trust Center
Trust Center

Security certifications, compliance documentation, and audit information.
セキュリティ認証、コンプライアンス文書、監査情報を提供いたします。

Browse Trust Center
Trust Centerを探索

## Section 03: Core Guides

Core guides for enterprise AI
エンタープライズAIのための

enablement
コアガイド

Start with the documents that explain the operating model, the failure of redaction-based approaches, and the architecture behind usable AI outputs.
運用モデル、墨消しベースのアプローチの限界、実用的なAI出力を支えるアーキテクチャを説明する文書からご覧ください。

Secure Enterprise AI Data Workflows
エンタープライズAIデータワークフローのセキュリティ

The end-to-end operating model for protecting sensitive data across AI workflows while restoring usable outputs locally.
AIワークフロー全体で機密データを保護しながら、使用可能な出力をローカルで復元するエンドツーエンドの運用モデルです。

Read guide
ガイドを読む

What Is an AI Data Capsule?
AIデータカプセルとは？

The core concept behind local encapsulation, protected AI processing, and local restoration.
ローカルカプセル化、保護されたAI処理、ローカル復元の基本概念を説明いたします。

Why Redaction Breaks Enterprise AI Workflows
墨消し（Redaction）がエンタープライズAIワークフローを破壊する理由

Why masking and redaction tools fail in AI workflows, and what replaces them in enterprise production settings.
マスキングおよび墨消しツールがAIワークフローで失敗する理由と、エンタープライズ運用環境における代替手法を説明いたします。

## Section 04: CTA Band

See how LLM Capsule works with your data
LLM Capsuleが実際のデータでどのように機能するかをご確認ください

Bring your documents, deployment constraints, and evaluation criteria. We demonstrate on your actual workflows.
実際の文書、デプロイ条件、評価基準をお持ちください。実際のワークフローで直接実演いたします。

Request a Demo
デモリクエスト

Available on AWS Marketplace
AWS Marketplaceで見る

...
LLM Capsule 대시보드 — 실시간 캡슐화 파이프라인과 문서 처리 현황
...
설계부터 모델 불문입니다. ChatGPT, Claude, Gemini, Perplexity, 또는 어떤 API든 벤더 종속 없이 사용합니다. 어떤 모델을 선택하든 보호는 동일하게 유지됩니다.

## Section 03: Zero Exposure

Encapsulate before outbound — raw data never
외부 전송 전 캡슐화 — 원본 데이터는 절대
...
문서 캡슐화 — 민감 데이터가 토큰으로 대체된 계약서의 전후 비교
...
LLM Capsule은 데이터가 외부로 나가기 전에 민감 엔티티를 로컬에서 캡슐화합니다. AI 처리에는 보호된 캡슐만 전달됩니다. 데이터 레이어에서의 로컬 캡슐화 — API 필터링이나 프롬프트 스캐닝과는 근본적으로 다릅니다.
...
원본 데이터는 기업 환경 내부에 머뭅니다. 민감 요소는 외부 전송 전에 탐지·대체됩니다.
...
[PERSON_01]이 [DATE_02]에 [ADDR_03] 소재 부동산에 대해 클레임 #[ID_04]를 제출했습니다. 총 평가 손해액: [AMOUNT_01].
...
James Mitchell이 2025년 3월 5일 Chicago, 42 Lincoln Ave 소재 부동산에 대해 클레임 #INS-2024-0847을 제출했습니다. 총 평가 손해액: $247,000.

## Section 05: Enterprise Context

Control what matters to your business — beyond generic
우리 회사에 중요한 것을 직접 통제합니다 — 일반
...
팀, 데이터 유형, 문서 분류, 워크플로우 맥락별로 캡슐화 정책을 정의하고 적용합니다.
...
ChatGPT, Claude, Gemini, Perplexity, 또는 어떤 LLM API든 — 어떤 모델을 선택하든 보호는 동일하게 유지됩니다.
...
LLM Capsule API 콘솔 — 기업 문서 관리 시스템과의 SDK 연동
...
결과물이 로컬에서 자동 복원되어 바로 쓸 수 있는 워크플로우를 만듭니다
...
PII를 넘어서는 Enterprise Context 제어
...
아닙니다. LLM Capsule은 데이터가 기업 환경을 벗어나기 전에 민감 엔티티를 로컬에서 캡슐화합니다. 외부 AI 모델은 보호된 캡슐만 처리합니다 — 원본 데이터는 절대 신뢰 경계를 넘지 않습니다. 원본과 캡슐화된 값의 매핑은 전적으로 고객 환경 내에 저장되며 외부로 전송되지 않습니다.
...
복원은 AI 처리 완료 후 전적으로 고객 통제 환경 내부에서 이루어집니다. 로컬에 저장된 매핑이 AI 결과물에 자동으로 적용되어 원본 이름, 수치, 날짜, 참조번호를 복원합니다. 업무 워크플로우에 바로 쓸 수 있는 기업용 결과물이 만들어집니다 — 수동 재구성이 필요 없습니다.

Can we define our own sensitive entities?
자체 민감 엔티티를 정의할 수 있나요?
...
가능합니다. LLM Capsule은 Enterprise Context 제어를 지원하므로 표준 PII 범주를 넘어 업무별 기밀 마커를 정의할 수 있습니다. 내부 식별자, 프로젝트명, 고객별 용어, 계약 참조번호, 재무 수치, 취약점 라벨, 조직별 민감도 범주를 포함합니다.
...
지원합니다. LLM Capsule은 PDF, Word 문서, 스프레드시트, 표, 프레젠테이션, 보고서, 혼합 형식 비즈니스 콘텐츠 등 기업 문서 유형 전반에 걸쳐 구조 보존 처리를 지원합니다. 캡슐화 과정에서 문서 구조, 엔티티 관계, 상호 참조가 유지됩니다.
...
가능합니다. LLM Capsule은 배포 가능한 컴포넌트로 구축되어 있으며, 기존 기업 포털, 파트너 백엔드, 고객 대면 애플리케이션, 내부 워크플로우 플랫폼에 API·SDK 기반 연동을 지원합니다. 데이터 레이어에서 캡슐화와 복원으로 기존 AI 연동을 감쌉니다.

Is AWS Marketplace deployment available?
AWS Marketplace 배포가 가능한가요?
...
가능합니다. LLM Capsule은 AWS Marketplace에서 간소화된 기업 조달, 빌링, VPC 배포를 위해 제공됩니다. 표준 기업 조달 워크플로우로 기존 AWS 환경 내에 신속하게 배포할 수 있습니다.

Can it run in air-gapped or on-premise environments?
망분리 또는 온프레미스 환경에서 실행할 수 있나요?
...
가능합니다. LLM Capsule은 온프레미스, 망분리, 클라우드, 하이브리드, 임베디드 배포 모델을 지원합니다. 망분리 환경에서는 격리 네트워크에서 문서를 캡슐화하고, 통제된 채널로 AI 처리를 위해 전송하며, 결과를 다시 가져와 로컬에서 복원합니다.

What AI models does LLM Capsule support?
LLM Capsule은 어떤 AI 모델을 지원하나요?
...
LLM Capsule은 모델 불문입니다. 모델 레이어가 아닌 데이터 레이어에서 동작하므로 ChatGPT, Claude, Gemini, Perplexity, 또는 어떤 LLM API든 모든 외부 AI 서비스와 호환됩니다. 모델 벤더가 바뀌어도 보호는 안정적으로 유지됩니다.

# Structure-Preserving Processing — 日本語翻訳
...
正確なAI処理のために、文書レイアウト、エンティティ関係、意味的コンテキストを維持しながら機密要素を置き換えます — 以下の製品のコア機能です —
...
構造保持処理は、文書レイアウト、エンティティ関係、テーブル構造、相互参照、意味的コンテキストを維持しながら、企業文書の機密要素を置き換えるデータ保護手法です。AIモデルが正確な処理のために構造的に完全な文書を受け取ることを保証します。
...
フラットマスキングは、すべての機密値を独立したトークンとして扱い、それぞれを汎用的なプレースホルダーに置き換えます。これにより、エンティティ関係が崩壊し、テーブルスキーマが破壊され、AIモデルが理解に使用する構造的情報が除去されます。
...
構造保持処理はこれらの関係を維持します。同一のエンティティが複数の場所に出現する場合、すべての出現箇所が同じ置き換え値にマッピングされます。テーブルの列ヘッダーと行識別子は意味的に一貫性を保ちます。文書セクション間の相互参照は論理的な接続を維持します。
...
契約書が条項全体にわたって複数の当事者を参照しています。構造保持処理は文書全体を通じて取得者にENTITY_Aを割り当て、AI分析のための一貫したエンティティ追跡を維持します。
...
カプセル化およびAI処理パイプライン全体を通じて、企業文書が完全な構造的整合性をどのように維持するかをご体験ください。

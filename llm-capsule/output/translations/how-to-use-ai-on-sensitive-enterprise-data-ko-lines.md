...
기업은 계약서, 재무 보고서, 의료 기록, 법률 서류, HR 커뮤니케이션, 내부 전략 메모 등 방대한 양의 민감 문서를 생성합니다. 이 문서들에는 AI가 변환할 수 있는 바로 그 정보가 담겨 있습니다 — 핵심 조항 추출, 분기 실적 요약, 환자 기록 분류, 기술 사양 번역.
...
하지만 이 데이터를 외부 AI 서비스로 전송하면 노출됩니다. LLM API로 전송되는 모든 프롬프트, 모든 문서, 모든 쿼리가 AI 제공자의 인프라에 그대로 노출됩니다. 규제 산업과 보안 중시 조직에서 이는 근본적인 충돌을 야기합니다 — AI로부터 가장 큰 혜택을 받는 데이터가 바로 노출할 수 없는 데이터입니다.
...
민감 데이터는 기업을 떠나기 전에 식별되고 대체되어야 합니다. 이는 단순 키워드 매칭이 아닙니다 — 기업 문서에는 맥락에 따른 민감도가 존재합니다. 계약서의 이름은 공개 디렉토리의 동일한 이름과 다른 위험을 수반합니다. 문서 구조 보존 처리가 AI 이해를 위해 문서의 의미적 관계를 온전하게 유지합니다.
...
2단계: 로컬 캡슐화. 감지된 민감 요소가 로컬에서 구조 보존 토큰으로 대체됩니다. 문서는 테이블·참조·계층 구조 등 전체 의미 구조를 유지하여 AI 모델이 정확하게 처리할 수 있습니다. 원본 값과 토큰 간의 매핑은 로컬에 저장되며 절대 외부로 전송되지 않습니다.
...
3단계: AI 처리. 캡슐화된 문서만 신뢰 경계를 넘습니다. AI 제공자는 보호된 값이 포함된 구조적으로 완전한 문서를 수신합니다. 요약·추출·분류·분석이 가능하지만 원본 민감 데이터는 절대 볼 수 없습니다.
...
4단계: 로컬 복원. 저장된 매핑을 사용하여 AI 출력이 로컬에서 복원됩니다. 실제 이름, 수치, 날짜, 참조가 자동으로 복원됩니다. 결과는 수동 후처리 없이 기업 워크플로우에서 직접 사용할 수 있는 비즈니스 준비 문서입니다.

Key principle: Your data stays on-premise. Only capsules travel. Outputs come back complete.
핵심 원칙: 데이터는 온프레미스에 유지됩니다. 캡슐만 이동합니다. 출력은 완전한 형태로 돌아옵니다.
...
LLM Capsule은 계약서, 재무 문서, 의료 기록, 법률 서류, HR 파일, 고객 커뮤니케이션을 포함한 정형 및 비정형 기업 데이터를 처리합니다.
...
네. LLM Capsule은 모델에 구애받지 않습니다. 전송 전 데이터 레이어에서 작동하므로 ChatGPT, Claude, Gemini, Perplexity 등 모든 LLM API와 함께 작동합니다.

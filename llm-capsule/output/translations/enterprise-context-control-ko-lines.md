# Enterprise Context Control — 한국어 번역
...
AI 처리 시 문서 유형·부서·워크플로우 맥락에 따라 보호 대상 데이터를 결정하는 정책 기반 민감도 분류입니다.
...
조직 정의 민감정보는 AI 워크플로우에서 민감도 분류에 정책 기반으로 접근합니다. 모든 데이터에 동일한 규칙을 일괄 적용하는 대신, 문서 유형·부서 출처·워크플로우 단계·기업별 민감도 정책에 따라 보호 대상을 결정합니다. 맥락 인식 데이터 제어의 핵심 구성 요소입니다.
...
같은 데이터라도 맥락에 따라 민감도가 달라집니다. 보도자료의 회사명은 공개 정보입니다. 동일한 회사명이 M&A 대상 목록에 있으면 매우 민감한 정보가 됩니다. 조직 정의 민감정보는 이러한 맥락별 차이를 반영한 보호 정책을 직접 정의할 수 있게 합니다.
...
PII 탐지에서 그치지 않습니다. 영업 비밀·재무 조건·전략 데이터·내부 지표, 그리고 조직 고유의 모든 민감도 범주를 포괄합니다 — 진정한 기업 기밀 제어를 제공합니다.
...
한 은행이 세 가지 문서 유형에 각각 다른 캡슐화 정책을 정의합니다. 고객 서신(PII만 보호), 내부 리스크 평가(PII·리스크 점수·한도 권고 보호), 이사회 자료(재무 전망 전체·전략 논평 보호). 조직 정의 민감정보가 각 문서에 알맞은 정책을 자동으로 적용합니다.
...
LLM Capsule (titleProduct)
LLM Capsule

Enterprise Context Control in Action (titlePost)
Enterprise Context Control, 직접 확인합니다

Experience how policy-based sensitivity classification protects enterprise data across every document type and workflow.
정책 기반 민감도 분류가 모든 문서 유형과 워크플로우에서 기업 데이터를 보호하는 방법을 직접 확인하세요.
